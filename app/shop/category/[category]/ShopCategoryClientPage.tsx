...
export default function ShopCategoryClientPage({ category }: { category: string }) {
  ...
  // Extract unique collections for this category
  const collections = Array.from(
    new Set(allProducts.filter((product) => product.category === category).map((product) => product.collection)),
  )
  ...
  // Apply filters
  useEffect(() => {
    let result = allProducts.filter((product) => product.category === category)
  ...
  }, [category, priceRange, selectedCollections])
    ...
    <main className="pt-24 pb-16 bg-white">
      <div className="container mx-auto px-4">
        ...
        <div className="max-w-3xl mx-auto text-center mb-12">
          <h1 className="font-serif text-4xl md:text-5xl lg:text-6xl text-amber-900 mb-6">{category}</h1>
          <p className="text-lg text-amber-800">Browse our divine collection of {category.toLowerCase()} products.</p>
        </div>
              ...
              <SheetContent className="w-full sm:max-w-md overflow-y-auto">
                ...
                <div className="py-6 space-y-8">
                  ...
                  {collections.length > 1 && (
                    <div>
                      <h3 className="font-medium text-amber-900 mb-4">Collection</h3>
                      <div className="space-y-3">
                        {collections.map((collection) => (
                          <div key={collection} className="flex items-center space-x-2">
                            ...
                            />
                            <Label htmlFor={`collection-${collection}`} className="text-amber-800 cursor-pointer">
                              {collection
                                .split("-")
        ...
        {/* Active filters */}
        {(selectedCollections.length > 0 || priceRange[0] > 0 || priceRange[1] < 5000) && (
